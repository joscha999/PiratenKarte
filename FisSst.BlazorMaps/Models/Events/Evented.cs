...
namespace FisSst.BlazorMaps
{
    ...
    public abstract class Evented : JsReferenceBase
    {
        ...
        private const string ZoomLevelChange = "zoomend";
        ...
        private readonly IDictionary<string, Func<Event, Task>> Events = new Dictionary<string, Func<Event, Task>>();
        ...
        public async Task OnZoomLevelChange(Func<Event, Task> callback)
            => await OnEvent(ZoomLevelChange, callback);
        ...
        private async Task OnEvent(string eventType, Func<Event, Task> callback) {
            if (Events.ContainsKey(eventType))
                return;

            Events.Add(eventType, callback);
            await OnEvent(eventType);
        }
        ...
        private async Task OnEvent(string eventType) {
            DotNetObjectReference<Evented> eventedClass = DotNetObjectReference.Create(this);
            await this.EventedJsInterop.OnCallbackEvent(eventedClass, this.JsReference, eventType);
        }
        ...
        [JSInvokable]
        public async Task OnCallbackEvent(string eventType, Event e) {
            if (Events.TryGetValue(eventType, out var callback))
